...
def jpeg_ghost_analysis(image_pil, qualities=range(50, 101, 5)):
    ...
    if np.any(ghost_map > 0):
        ghost_map = np.log1p(ghost_map)  # Logarithmic scaling to reduce contrast of high-intensity areas
        ghost_map = (ghost_map - np.min(ghost_map)) / (np.max(ghost_map) - np.min(ghost_map) + 1e-6) # Normalize to 0-1 range
